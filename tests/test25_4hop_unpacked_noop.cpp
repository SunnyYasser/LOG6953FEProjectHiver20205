...
ulong pipeline_example(const std::string &query) {
    ...
    const auto parser = std::make_unique<VFEngine::QueryParser>(
            query, column_ordering, false, VFEngine::SinkType::NO_OP, column_names, column_alias_map);
    ...
    const std::vector<std::string> operator_names{"SCAN", "INLJ1", "INLJ2", "INLJ3", "INLJ4", "SINK_NO_OP"};
    int idx = 0;
