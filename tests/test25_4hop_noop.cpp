...
ulong pipeline_example(const std::string &query) {
    ...
    const auto parser = std::make_unique<VFEngine::QueryParser>(query, column_ordering, VFEngine::SinkType::NO_OP,
    ...
    const std::vector<std::string> operator_names{"SCAN", "INLJ1", "INLJ2", "INLJ3", "INLJ4", "SINK_NOOP"};
    int idx = 0;
...
int main() {
    ...
    std::cout << "Test 25: " << query << std::endl;
    ...
    if (actual_result_test != expected_result_test) {
        std::cerr << "Test 25 failed: Expected " << expected_result_test << " but got " << actual_result_test
                  << std::endl;
        return 1;
    }
