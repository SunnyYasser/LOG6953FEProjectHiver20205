...
ulong test_17(const std::string &query) { return pipeline_example(query); }
...
int main() {
    ...
    std::cout << "Test 17: " << query << std::endl;
    const auto start = std::chrono::high_resolution_clock::now();
    const auto expected_result_test_17 = get_expected_value();
    const auto actual_result_test_17 = test_17(query);
    ...
    if (actual_result_test_17 != expected_result_test_17) {
        std::cerr << "Test 17 failed: Expected " << expected_result_test_17 << " but got " << actual_result_test_17
                  << std::endl;
        return 1;
    }
