...
ulong pipeline_example(const std::string &query) {
    ...
    const auto parser =
            std::make_unique<VFEngine::QueryParser>(query, column_ordering, VFEngine::SinkType::PACKED, column_names, column_alias_map);
...
ulong test_7(const std::string &query) { return pipeline_example(query); }
...
int main() {
    ...
    std::cout << "Test 7: " << query << std::endl;
    const auto start = std::chrono::high_resolution_clock::now();
    const auto expected_result_test_7 = get_expected_value();
    const auto actual_result_test_7 = test_7(query);
    ...
    if (actual_result_test_7 != expected_result_test_7) {
        std::cerr << "Test 7 failed: Expected " << expected_result_test_7 << " but got " << actual_result_test_7
                  << std::endl;
        return 1;
    }
