...
namespace VFEngine {
    ...
    public:
        ...
        [[nodiscard]] std::shared_ptr<FactorizedTreeElement> build_tree(bool is_packed) const;

    private:
        std::vector<LogicalPipelineElement> _logical_plan;
        [[nodiscard]] std::shared_ptr<FactorizedTreeElement> insert(const std::shared_ptr<FactorizedTreeElement> &root,
