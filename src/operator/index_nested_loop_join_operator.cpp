...
namespace SampleDB
{
    ...
    void IndexNestedLoopJoin::execute_in_chunks()
    {
        ...
        while (_input_vector.get_pos() < _input_vector.get_size())
        {
            ...
            auto& newdata_values = _datastore->get_values_from_index(data[_data_idx]);
            ...
            while (start_idx < newdata_values.size())
            {
                ...
                log_vector(_output_vector, get_operator_name_as_string (get_operator_type (), get_uuid ()), "INLJ::execute()");
